...
namespace experimental
{
	...
	{
	public:
		static const size_t max_size = width * height;
	...
	private:
		...
		size_t position_to_index(const Position& at) const
		{
			...
			return (static_cast<size_t>(at.m_y) * width) + at.m_x;
		}
	...
	public:
		...
		size_t size() const
		{
			return position_to_index({ width, height });
		}
		...
		void display()
		{
			for(PointInt y = 0; y < height; y++)
			{
				for(PointInt x = 0; x < width; x++)
				{
		...
		template<const size_t width, const size_t height>
		BitMatrix<width, height> operator&(const BitMatrix<width, height>& other)
		{
	...
	typedef BitMatrix<0, 0> BitMatrixTypes;
}
...
public:
	...
	BitMatrix operator&(const BitMatrix& other)
	{
		BitMatrix to_return = this->m_bits;
		return to_return &= other;
	}
	Scanner& operator|=(const BitMatrix& other)
	{
		this->m_bits |= other;
		return *this;
	}
